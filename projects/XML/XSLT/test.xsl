	...
	<xsl:template match="ServiceData/MasterData/Expenses">
		<TechnionData>
			<File001>
				...
				<Tikzuv>
					<xsl:variable name="var:v1" select="user:converttype(string(ancestor::*[1]/@BudgetType))"/>
					...
					<Expenses-002>
						...
						<xsl:for-each select="TotalExpenses">
							<PirutExpenses_003>
								...
								<xsl:value-of select="./text()"/>
								<ExpDebCrd>
									<xsl:variable name="var:v2" select="user:convert2(string(@HovaZhut))"/>
									<xsl:value-of select="$var:v2"/>
								</ExpDebCrd>
								<ExpUpdated>
									<xsl:variable name="var:v3" select="user:convertDate2(string(@UpdateDate))"/>
									<xsl:value-of select="$var:v3"/>
								</ExpUpdated>
								<ExpValueDate>
									<xsl:variable name="var:v4" select="user:convertDate(string(@ValueDate))"/>
						...
						<Incomes002>
							...
							<xsl:for-each select="TotalIncomes">
								<PirutIncomess_003>
									...
									<xsl:value-of select="./text()"/>
									<IncDebCrd>
										<xsl:variable name="var:v5" select="user:convertInc(string(ancestor::*[3]/@HovaZhut))"/>
										<xsl:value-of select="$var:v5"/>
									</IncDebCrd>
									<IncUpdated>
										<xsl:variable name="var:v6" select="user:convertDateUpInc(string(ancestor::*[3]/@UpdateDate))"/>
										<xsl:value-of select="$var:v6"/>
									</IncUpdated>
									<IncValueDate>
										<xsl:variable name="var:v7" select="user:convertIncV(string(ancestor::*[3]/@ValueDate))"/>
...
function converttype(param1)
  if param1 <> "" then
     select case cint(param1)
         case 1
              converttype="פעילות לא מתוקצבת"
         case 0
              converttype="פעילות מתוקצבת"
     end select
  end if
end function
]]>
