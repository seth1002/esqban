   ...
   <xsl:template match="ServiceData/BL_NLFR_REPORT/LIST_G_PAYING_CUST_NO/G_PAYING_CUST_NO">
      <G_PAYING_CUST_NO>
                                       ...
                                       <xsl:for-each select="G_TERM_CODE">
                                          <G_TERM_CODE>
                                             ...
                                             <xsl:for-each select="TOTAL_PRICE_BEFOR_HANACHA">
                                                <TOTAL_PRICE_DOLAR>
                                                   <!--Connection from Source Node "TOTAL_PRICE_BEFOR_HANACHA" to Destination Node "TOTAL_PRICE_DOLAR"-->
                                                   <xsl:value-of select="./text()"/>
                                                </TOTAL_PRICE_DOLAR>
                                             </xsl:for-each>
         ...
         <xsl:for-each select="CP_MONTH_XML">
            <MON>
               <xsl:variable name="var:v1" select="user:fncmonth(string(ancestor::*[2]/CP_MONTH_XML./text()))"/>
               <xsl:value-of select="$var:v1"/>
            </MON>
         </xsl:for-each>
      </G_PAYING_CUST_NO>
   </xsl:template>
<msxsl:script language="VBScript" implements-prefix="user"><![CDATA[function convertDate(param1)
    convertDate=mid(param1, 3, 2) 
end function
