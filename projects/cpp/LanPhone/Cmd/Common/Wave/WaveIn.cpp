...
int CWaveIn::Start()
{
	...
	waveInThread = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)waveInProc, this, 0, &dwThreadId);
	if (!waveInThread)
	{
	...
	ZeroMemory(&m_WaveHeader[0], sizeof(WAVEHDR) * 2);
	...
	err = waveInOpen(&m_WaveInHandle, WAVE_MAPPER, &waveFormat, dwThreadId, 0, CALLBACK_THREAD);
	if (err)
	{
	...
	else
	{
		...
		err = mixerOpen(&m_MixerHandle, (DWORD)m_WaveInHandle, 0, 0, MIXER_OBJECTF_HWAVEIN);
		if (err)
		{
		...
		else
		{
			...
			err = mixerGetLineInfo((HMIXEROBJ)m_MixerHandle, &mixerLine, MIXER_GETLINEINFOF_COMPONENTTYPE);
			if (err)
...
//			gOutput = fopen(argv[1], "wb");
		...
		m_WaveHeader[1].dwBufferLength = m_WaveHeader[0].dwBufferLength = waveFormat.nAvgBytesPerSec / 100;//<< 1;
		if (!(m_WaveHeader[0].lpData = (char *)VirtualAlloc(0, m_WaveHeader[0].dwBufferLength * 2, MEM_COMMIT, PAGE_READWRITE)))
		{
		...
		else
		{
			...
			m_WaveHeader[1].lpData = m_WaveHeader[0].lpData + m_WaveHeader[0].dwBufferLength;

			// Leave other WAVEHDR fields at 0

			// Prepare the 2 WAVEHDR's
			if ((err = waveInPrepareHeader(m_WaveInHandle, &m_WaveHeader[0], sizeof(WAVEHDR))))
			{
				fprintf(stderr, "Error preparing WAVEHDR 1! -- %08X\n", err);
			}
			else
			{
				if ((err = waveInPrepareHeader(m_WaveInHandle, &m_WaveHeader[1], sizeof(WAVEHDR))))
				{
				...
				else
				{
					...
					if ((err = waveInAddBuffer(m_WaveInHandle, &m_WaveHeader[0], sizeof(WAVEHDR))))
					{
					...
					else
					{
						...
						if ((err = waveInAddBuffer(m_WaveInHandle, &m_WaveHeader[1], sizeof(WAVEHDR))))
						{
						...
						else
						{
							...
							m_bRecording = TRUE;
							if ((err = waveInStart(m_WaveInHandle)))
							{
	...
	do
	{
		err = waveInClose(m_WaveInHandle);
...
//		fclose(gOutput);
	...
	if (m_WaveHeader[0].lpData) VirtualFree(m_WaveHeader[0].lpData, 0, MEM_RELEASE);
