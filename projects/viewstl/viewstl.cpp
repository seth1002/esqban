...
void DrawSolidBody(CSolidBody *pbody)
{
	if( pbody->selected() )
	{
	...
	for(int x=0; x<pbody->size(); x++)
	{
		...
		Facet *pfacet = &(*pbody)[x];
		glBegin(GL_POLYGON);
		glNormal3f(pfacet->normal.x, pfacet->normal.y, pfacet->normal.z);
		for(int i=0; i<3; i++)
		{
			Vertex &vertex = pfacet->vertex[i];
...
void DrawGLScene()
{
	...
	float color1[] = { 0.4f, 0.4f, 1.0f, 1.0f };
	float color2[] = { 1.0f, 0.0f, 0.0f, 1.0f };
	int counter = 0;
	std::vector<CSolidBody>::iterator it;
	for(it=bodies.begin(); it<bodies.end(); it++)
	{
		float *pcolor = counter%2 ? color2 : color1;
...
int main(int argc, char *argv[])
{
	...
	{
		//body.LoadSTL( "1920430_B.stl" ); bodies.push_back(body);
		//body.LoadSTL( "1920430_I.stl" ); bodies.push_back(body);

		body.LoadSTL( "Shaped_turn_tool2.STL" ); bodies.push_back(body);

		//body.LoadSTL( "636106.stl" ); bodies.push_back(body);
		//body.LoadSTL( "636107.stl" ); bodies.push_back(body);
	}
